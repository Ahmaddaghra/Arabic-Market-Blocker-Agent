...
export async function createPlan(fields:Field[],buttons:string[]):Promise<{supported:boolean;reason:string;actions:PlanAction[];source:'gpt-5.6'|'deterministic-fallback'}>{
  const fallback=()=>{const actions:PlanAction[]=fields.filter(f=>['text','email','tel','password'].includes(f.type)).map(f=>({action:'fill',fieldPurpose:f.type,locator:f.locator,valueKey:f.type==='email'?'email':f.type==='tel'?'phoneLocal':f.type==='password'?'password':'fullName',reason:`Test ${f.label||f.name||f.type} with the Saudi persona`}));return {supported:fields.length>=2,reason:fields.length>=2?'Standard form fields detected.':'No standard signup form fields were detected.',actions,source:'deterministic-fallback' as const}};
  if(process.env.OPENAI_API_KEY){
    try{
      const client=new OpenAI();
      const response=await client.responses.create({model:process.env.OPENAI_MODEL||'gpt-5.6',reasoning:{effort:'low'},input:[{role:'system',content:'You plan a bounded, non-destructive audit of a public standard signup form. Use only supplied DOM-grounded locators. Never bypass CAPTCHA, authentication, or submit a real account. Compare an English baseline with Saudi Arabic persona input. Return the smallest useful plan.'},{role:'user',content:JSON.stringify({market,fields,buttons})}],text:{format:{type:'json_schema',name:'signup_audit_plan',strict:true,schema:planSchema}}},{signal:AbortSignal.timeout(15_000)});
      ...
      console.warn('GPT planner unavailable; using deterministic fallback.',error instanceof Error?error.message:'Unknown planner error');
      return fallback();
    }
  }
  return fallback();
